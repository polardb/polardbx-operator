      ...
      <mat-card>
        ...
        <mat-card-content>
          ...
          <div class="form-row" *ngIf="!loadingPods">
            <label>目标 Pod</label>
            <mat-select [(ngModel)]="targetPod" placeholder="选择运行中的非 leader Pod">
              <mat-option *ngFor="let p of filteredPods" [value]="p?.metadata?.name">{{ p?.metadata?.name }}</mat-option>
            </mat-select>
          </div>
          <div class="form-row"><label>源 XStore(可选)</label><input [(ngModel)]="fromXStore" placeholder="cluster-dn-0 源所属XStore"/></div>
          ...
          <div class="actions">
            <button mat-raised-button color="primary" (click)="submit()" [disabled]="submitting">
              <mat-icon>play_arrow</mat-icon> {{ submitting ? '提交中...' : '发起重搭' }}
            </button>
          </div>
        </mat-card-content>
      </mat-card>
...
export class XStoreRebuildFollowerComponent {
  ...
  submit(): void {
    ...
    const req: any = {
      ...
      targetPodName: this.targetPod?.trim() || undefined,
      fromXStore: this.fromXStore?.trim() || undefined
    };
    this.submitting = true;
    this.api.rebuildFollower(this.namespace || 'default', req).subscribe({
      next: () => { this.snack.open('已提交 follower 重搭', '关闭', { duration: 3000 }); this.submitting = false; this.router.navigateByUrl('/storage/xstore-followers'); },
  ...
  isValid(): boolean {
    return !!(this.namespace && this.xstore && this.targetPod);
  }
}