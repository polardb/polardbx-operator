...
@Component({
  ...
  template: `
    <div class="page-wrapper recovery-wizard">
      <div class="page-header">
        <div class="title-block">
          ...
          <p>分步式引导完成集群恢复操作，支持备份恢复和时间点恢复</p>
        </div>
      </div>
  ...
  styles: [`
    .page-wrapper {
      ...
      padding: 24px;
      min-height: 100%;
      background: transparent; /* 外层背景由 layout 负责 */
    }
  ...
  async executeRestore(): Promise<void> {
    ...
    try {
      ...
      if (summary.restoreType === 'pitr') {
        let restoreTime = '' as string;
        const t: any = summary.pitrTime as any;
        const tz = String(summary.pitrTimezone || 'UTC').trim() || 'UTC';
        ...
        restoreTime = this.formatRestoreTime(d, tz);
