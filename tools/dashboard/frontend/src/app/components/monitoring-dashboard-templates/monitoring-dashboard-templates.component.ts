...
@Component({
  ...
  template: `
    <div class="page-wrapper dashboard-templates" *ngIf="initialized">
      <div class="page-header">
        <div class="title-block">
          <h2>
            <i nz-icon nzType="appstore" nzTheme="outline" class="page-icon"></i>
            Dashboard 模板
          </h2>
          <p>
            Grafana 仪表板模板（导入将写入 ConfigMap <code>polardbx-grafana-dashboards</code>）
      ...
      <nz-alert
        *ngIf="!templatesDirReadable"
        nzType="warning"
        nzShowIcon
        nzMessage="后端未读取到模板目录"
        nzDescription="当前仅展示模板清单（待同步）。请确认后端镜像包含 charts/polardbx-monitor/dashboard，或设置环境变量 POLARDBX_GRAFANA_TEMPLATES_DIR 指向模板目录。">
      </nz-alert>
      ...
      <div class="filters">
        <div class="filter-left">
          ...
          <label class="only-available" nz-checkbox [(ngModel)]="onlyAvailable" (ngModelChange)="applyFilters()">
            仅显示可导入模板
          </label>
        </div>
      </div>
  ...
  styles: [`
    ...
    .filter-item.search nz-input-group {
      width: 100%;
    }
...
export class MonitoringDashboardTemplatesComponent implements OnInit {
  ...
  keyword = '';
  // 默认展示全部模板，避免“总数有但列表空”的困惑；用户可自行勾选“仅可导入”过滤
  onlyAvailable = false;
  stats: TemplateStats = { total: 0, available: 0, recommended: 0, missing: 0 };
  directory = '';
  templatesDirReadable = true;
  private showedTemplatesDirToast = false;
  ...
  private loadData(force = false): void {
    ...
    this.templatesDirReadable = true;
    this.loading = true;
    ...
    const summaries$ = this.api
      ...
      .pipe(catchError(err => {
        ...
        this.templatesDirReadable = false;
        // 提示但仍展示模板清单（待同步）；避免每次刷新都弹 toast
        if (!this.showedTemplatesDirToast) {
          this.message.warning('无法读取 Grafana 模板目录：将展示模板清单（可用“待同步”模板）。请确认后端镜像包含 charts 目录。');
          this.showedTemplatesDirToast = true;
        }
    ...
    forkJoin({ manifest: manifest$, summaries: summaries$ })
      ...
      .subscribe(({ manifest, summaries }) => {
        ...
        // 后端不可用时，确保不会因为“仅可导入”导致列表被过滤为空
        if (!this.templatesDirReadable && this.onlyAvailable) {
