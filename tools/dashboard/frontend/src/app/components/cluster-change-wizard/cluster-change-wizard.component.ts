...
export class ClusterChangeWizardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.getCluster(this.namespace, this.name).subscribe({
      next: (c: any) => {
        this.currentVersion = (c?.spec?.topology?.version || c?.status?.version || c?.status?.polardbxVersion || c?.spec?.version || '').toString();
        const cn = c?.spec?.topology?.nodes?.cn?.replicas;
        const dn = c?.spec?.topology?.nodes?.dn?.replicas;
        if (typeof cn === 'number' && typeof dn === 'number') {
          this.scaleForm.patchValue({ cnReplicas: cn, dnReplicas: dn }, { emitEvent: false });
        }
      },
      error: () => {}
    });
