...
var _ = ginkgo.Describe("[PolarDBXMonitor] [Lifecycle:Create]", func() {
	f := framework.NewDefaultFramework(framework.TestContext)

	ginkgo.It("should polardbxmonitor be monitoring after creating", func() {
	...
	ginkgo.It("should polardbxmonitor support user defined monitor interval and scrape timeout", func() {
		pxc := pxcframework.NewPolarDBXCluster(
			"e2e-test-monitor-interval",
			f.Namespace,
			pxcframework.ProtocolVersion(5),
			pxcframework.TopologyModeGuide("quick-start"),
		)

		pxm := pxmframework.NewPolarDBXMonitor(pxc.Name,
			pxc.Namespace,
