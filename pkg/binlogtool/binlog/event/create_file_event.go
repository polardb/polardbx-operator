...
type CreateFileEvent struct {
	...
	BlockData         str.Str   `json:"block_data,omitempty"`
}

func (e *CreateFileEvent) Layout(version uint32, code byte, fde *FormatDescriptionEvent) *layout.Layout {
	...
	return layout.Decl(
		...
		layout.Area(layout.Infinite(), func(block []byte) (int, error) {
			nullIdx := bytes.IndexByte(block, 0)
			if nullIdx < 0 {
				return 0, fmt.Errorf("no null found")
			}
			e.File = block[:nullIdx]
			e.BlockData = block[nullIdx+1:]
