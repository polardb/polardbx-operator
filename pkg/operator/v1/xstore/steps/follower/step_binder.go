...
import (
	...
	reconcile2 "github.com/alibaba/polardbx-operator/pkg/operator/v1/xstore/reconcile"
	"github.com/go-logr/logr"
	"sigs.k8s.io/controller-runtime/pkg/reconcile"
)

type ConditionFunc func(rc *reconcile2.FollowerContext, log logr.Logger) (bool, error)
type StepFunc func(rc *reconcile2.FollowerContext, flow control.Flow) (reconcile.Result, error)
	...
	return control.NewStepBinder(
		control.NewStep(
			name, func(rc control.ReconcileContext, flow control.Flow) (reconcile.Result, error) {
				return f(rc.(*reconcile2.FollowerContext), flow)
			},
		),
	)
}
...
func NewStepIfBinder(conditionName string, condFunc ConditionFunc, binders ...control.BindFunc) control.BindFunc {
	condition := control.NewCachedCondition(
		control.NewCondition(conditionName, func(rc control.ReconcileContext, log logr.Logger) (bool, error) {
			return condFunc(rc.(*reconcile2.FollowerContext), log)
		}),
	)
